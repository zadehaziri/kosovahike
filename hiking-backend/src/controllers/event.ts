...
export class EventController {
    ...
    async deleteEvent(eventId: any, userId: any, isAdmin: boolean = false) {
        try {
            ...
            if (event.attendees && Array.isArray(event.attendees)) {
                for (const attendee of event.attendees) {
                    if (attendee._id) {
                        const attendeeUser = await UserModel.findById(attendee._id);
                        if (attendeeUser) {
                            attendeeUser.eventsAttending = attendeeUser.eventsAttending.filter(
                                (e) => e.toString() !== eventId.toString()
                            );
            ...
            throw error; // Re-throw to preserve custom error codes
        }
    }
    ...
    async joinEvent(eventId: string, userId: string) {
        try {
            ...
            if (!user) {
                ...
                const allUsers = await UserModel.find({}, '_id email firstName lastName');
            ...
            const customError: any = new Error(error?.message || "Internal Server Error");
